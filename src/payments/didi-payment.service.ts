...
export class DidiPaymentService {
  ...
  async preAuthorizePayment(
    ...
    this.logger.log(
      `Pre-autorizando pago de $${finalPrice} para viaje #${rideId}`,
    );

    // 2️⃣ Intentar cobro/validación según método
    if (method === 'card' && !paymentToken) {
      throw new BadRequestException(
        'Token de tarjeta requerido para pago con tarjeta',
      );
    }
   ...
   */
  private async chargeCard(token: string, amount: number, user: User) {
    if (!token) {
      throw new BadRequestException('Token de tarjeta requerido');
    }

    this.logger.log(
      `Cobrando $${amount} a tarjeta de ${user.firstName} ${user.lastName}`,
    );
  ...
  private async chargeWallet(userId: number, amount: number) {
    ...
    this.logger.log(
      `Descontados $${amount} de billetera de ${user.firstName} ${user.lastName}. Nuevo saldo: $${user.walletBalance}`,
    );
  ...
  private calculateFixedPrice(ride: Ride): number {
    ...
    this.logger.log(
      `Precio calculado: Base $${BASE_FARE} + Distancia $${distanceCharge} + Tiempo $${timeCharge} = $${totalPrice}`,
    );
   ...
   */
  async rechargeWallet(userId: number, amount: number, paymentToken: string) {
    if (amount < 5000) {
    ...
    if (!chargeResult.success) {
      throw new BadRequestException(
        'No se pudo procesar el cobro a la tarjeta',
      );
    }
    ...
    this.logger.log(
      `Recarga exitosa: +$${amount} a billetera de ${user.firstName} ${user.lastName}. Nuevo saldo: $${user.walletBalance}`,
    );
